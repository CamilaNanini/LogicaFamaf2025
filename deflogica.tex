...
\begin{enumerate}[label=(\arabic*)]
    ...
    Sea $T = (\Sigma, \tau)$ una teoría. Dada $\varphi \in S^{\tau}$ definamos
    \[
...
\section*{Combo 3}
\begin{enumerate}[label=(\arabic*)]
    \item Dados $t =_d t(v_1, \ldots, v_n) \in T^\tau$, $A$ una estructura de tipo $\tau$ y $a_1, \ldots, a_n \in A$, defina $t^A[a_1, \ldots, a_n]$ (i.e. Convención notacional 2)

    \bigskip
    
    Dados $t =_d t(v_1, \ldots, v_n) \in T^\tau$, $A$ una estructura de tipo $\tau$ y $a_1, \ldots, a_n \in A$, se define:
    \[
    t^A[a_1, \ldots, a_n]
    \]
    ...
    \[
    [S) = \{\, y \in L : y \geq s_1 i \dots i s_n, \text{ para algunos } s_1, \ldots, s_n \in S, \, n \geq 1 \,\}.
    \]
    Llamaremos a $[S)$ el \emph{filtro generado por} $S$.

    \item Defina cuando $J \in \text{Just}^+$ es balanceada (no hace falta que defina $B^J$)
    \bigskip
...
\begin{enumerate}
    ...
    \item Si $B_1, B_2 \in B^J$, entonces $B_1 \cap B_2 = \emptyset$ o $B_1 \subseteq B_2$ o $B_2 \subseteq B_1$.
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    Definimos recursivamente la relación “$v$ ocurre libremente en $\varphi$ a partir de $i$”, donde $v \in \text{Var}$, $\varphi \in F^\tau$ e $i \in \{1, \dots, |\varphi|\}$, de la siguiente manera:
...
\bigskip
    ...
    Si $t$ es un término de tipo $\tau$, entonces escribiremos 
    \[
    t =_d t(v_1, \ldots, v_n)
    \]
    para declarar que $v_1, \ldots, v_n$ son variables distintas (con $n \ge 1$) y tales que toda variable que ocurre en $t$ pertenece a $\{v_1, \ldots, v_n\}$ (no necesariamente toda $v_j$ debe ocurrir en $t$).
...
Si $\varphi$ es una fórmula de tipo $\tau$, entonces escribiremos 
\[
\varphi =_d \varphi(v_1, \ldots, v_n)
\]
para declarar que $v_1, \ldots, v_n$ son variables distintas (con $n \ge 1$) tales que 
\[
L_i(\varphi) \subseteq \{v_1, \ldots, v_n\}.
\]
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item Defina “$i$ es anterior a $j$ en $(\varphi, J)$” (no hace falta que defina $B^J$)
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \[
    A_T = (S_\tau / \dashv\vdash_T, \, s^T, \, i^T, \, c^T, \, 0^T, \, 1^T),
    \]
    donde:
    \[
    ...
    \]
    \[
    1^T = \{\varphi \in S_\tau : T \vdash \varphi\}, \quad 
    0^`T = \{\varphi \in S_\tau : T \vdash \neg \varphi\}.
    \]
    El álgebra $A_T$ es un álgebra de Boole.

    \item Defina “$S$ es un subuniverso del reticulado complementado $(L,s,i,c,0,1)$”
    Sea $(L, s, i, c, 0, 1)$ un reticulado complementado.  
    Un conjunto $S \subseteq L$ es llamado **subuniverso** de $(L, s, i, c, 0, 1)$ si $0,1 \in S$ y además $S$ es cerrado bajo las operaciones $s$, $i$ y $c$.
