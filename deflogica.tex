...
\begin{enumerate}[label=(\arabic*)]
    ...
    $\varphi_l$ es una hipótesis de $\varphi_i$ en $(\varphi, J)$, cuando haya en $B_J$ 
...
\section*{Combo 3}
\begin{enumerate}[label=(\arabic*)]
    \item Dados $t \stackrel{d}{=} t(v_1, \ldots, v_n) \in T_\tau$, $A$ una estructura de tipo $\tau$ y $a_1, \ldots, a_n \in A$, defina $t^A[a_1, \ldots, a_n]$ (i.e. Convención notacional 2)

    \bigskip
    
    Dados $t \stackrel{d}{=} t(v_1, \ldots, v_n) \in T_\tau$, $A$ una estructura de tipo $\tau$ y $a_1, \ldots, a_n \in A$, se define:
    \[
    t^A[a_1, \ldots, a_n]
    \]
    como el elemento $t^A[\vec{b}]$, donde $\vec{b}$ es una asignación tal que a cada variable $v_i$ le asigna el valor $a_i$.  
    Es decir, $t^A[a_1, \ldots, a_n] = t^A[\vec{b}]$ cuando $\vec{b}(v_i) = a_i$ para todo $i = 1, \ldots, n$.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    Dadas palabras $\alpha, \beta \in \Sigma^*$, con $|\alpha|, |\beta| \ge 1$, y un natural $i \in \{1, \dots, |\beta|\}$, se dice que $\alpha$ ocurre a partir de $i$ en $\beta$ cuando existen palabras $\delta, \gamma$ tales que $\beta = \delta \alpha \gamma$ y $|\delta| = i-1$.  
    O sea "$\alpha$ ocurre a partir de $i$ en $\beta$ si al comenzar a leer desde el $i$-ésimo lugar de $\beta$"
...
\bigskip
    ...
    t = f(t_1, \ldots, t_m),
    \]
    con $f \in F_m$ y $t_1, \ldots, t_m \in T_\tau$—, entonces supondremos tácitamente que también se han hecho las declaraciones:
    \[
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item \textbf{(1)} Si $\varphi = (t \equiv s)$, con $t, s \in T_\tau$, entonces también declaramos:
    \[
    t =_d t(v_1, \ldots, v_n) \quad \text{y} \quad s =_d s(v_1, \ldots, v_n).
    \]

    \item \textbf{(2)} Si $\varphi = r(t_1, \ldots, t_m)$, con $r \in R_m$ y $t_1, \ldots, t_m \in T_\tau$, entonces declaramos:
    \[
